...
onload = () => {
  ...
  render(
    html`${
      versions.map((item) => html`<option value=${item}>${item}</option>`)
    }`,
    versionEl,
  );
...
const renderEnabled = (enabled, version, versionData) =>
  html`
    ...
    enabled.map((item) =>
      ...
      ${
        ("motivation" in item)
          ? html`<p>${item.creator} created this feature because: <blockquote>${item.motivation}</blockquote></p>`
          : html``
      }
      ...
      <h4>Resources</h4>
      ${
        ("docs" in item.resources)
          ? html`<p>Docs: ${
            item.resources.docs.map((resource) =>
              html`<a href=${resource}>${resource}</a>`
            )
          }</p>`
          : html`No linked docs`
      }</p>
      ${
        ("samples" in item.resources)
          ? html`<p>Samples: ${
            item.resources.samples.map((resource) =>
              html`<a href=${resource}>${resource}</a>`
            )
          }</p>`
          : html`No linked samples`
      }</p>`
    )
  }`;

const renderOriginTrials = (enabled, version, versionData) =>
  html`
...
const renderFlaggedFeatures = (enabled, version, versionData) =>
  html`
...
const renderDeprecatedFeatures = (enabled, version, versionData) =>
  html`
...
const renderRemovedFeatures = (enabled, version, versionData) =>
  html`
